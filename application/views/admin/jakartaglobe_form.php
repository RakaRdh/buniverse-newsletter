<?php $this->load->view('admin/header', ['title' => $action === 'create' ? 'Create Jakarta Globe Content' : 'Edit Jakarta Globe Content']); ?>
    ...
    <div class="flex justify-between items-center mb-8 pb-4 border-b border-slate-100">
        <div>
            <h1 class="text-2xl font-bold text-[#20254D] flex items-center gap-2">
                <i class="fa-solid fa-file-pen text-orange-500"></i>
                <?= $action === 'create' ? 'Create Jakarta Globe Content' : 'Edit Jakarta Globe Content' ?>
            </h1>
            <p class="text-sm text-slate-500 mt-1">Configure layout, greeting briefing, sidebar links, and alternating topics</p>
        </div>
        <span class="px-3 py-1 bg-orange-50 text-orange-600 border border-orange-200 rounded-full text-xs font-bold uppercase tracking-wider">Jakarta Globe Template</span>
    </div>
    ...
    <?= form_open('newsletters/save', ['class' => 'space-y-8']) ?>
        ...
        <div class="space-y-5">
            <h3 class="text-sm font-bold text-[#20254D] uppercase tracking-wider flex items-center gap-2 pb-2 border-b border-slate-100">
                <span class="w-1.5 h-4 bg-orange-500 rounded-sm"></span> 1. General & Greeting Settings
            </h3>
        ...
        <?php if ($action === 'edit' && !empty($articles)): ?>
            ...
            <?php if ($main_art): ?>
                <div class="space-y-5 pt-4">
                    <h3 class="text-sm font-bold text-[#20254D] uppercase tracking-wider flex items-center gap-2 pb-2 border-b border-slate-100">
                        <span class="w-1.5 h-4 bg-orange-500 rounded-sm"></span> 2. Featured Topic (Main Content)
                    </h3>
            ...
            <!-- Section 3: Sidebar Topics (1-3) -->
            <div class="space-y-6 pt-4">
                <h3 class="text-sm font-bold text-[#20254D] uppercase tracking-wider flex items-center gap-2 pb-2 border-b border-slate-100">
                    <span class="w-1.5 h-4 bg-orange-500 rounded-sm"></span> 3. Sidebar Topics (1-3)
                </h3>

                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <?php foreach ($sidebar_arts as $idx => $art): ?>
                        <?php $field_idx = $idx + 1; ?>
                        <div class="bg-slate-50/50 border border-slate-200 rounded-xl p-4 space-y-4">
                            <h4 class="text-xs font-bold text-slate-500 uppercase tracking-wider">Sidebar Item #<?= $field_idx ?></h4>
                            <input type="hidden" name="articles[<?= $field_idx ?>][id]" value="<?= $art['id'] ?>">
                            
                            <div>
                                <label class="block text-xs font-bold text-slate-655 mb-1">Title</label>
                                <input type="text" name="articles[<?= $field_idx ?>][title]" class="w-full px-2.5 py-1.5 text-xs bg-white border border-slate-200 rounded-lg text-slate-800 focus:outline-none" required value="<?= htmlspecialchars($art['title']) ?>">
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-slate-655 mb-1">Category</label>
                                <input type="text" name="articles[<?= $field_idx ?>][category]" class="w-full px-2.5 py-1.5 text-xs bg-white border border-slate-200 rounded-lg text-slate-800 focus:outline-none" required value="<?= htmlspecialchars($art['category']) ?>">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-655 mb-1">Article URL / Link</label>
                                <input type="url" name="articles[<?= $field_idx ?>][url]" class="w-full px-2.5 py-1.5 text-xs bg-white border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:border-[#20254D]" placeholder="https://example.com/article" value="<?= htmlspecialchars(isset($art['url']) ? $art['url'] : '') ?>">
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-slate-655 mb-1">Image URL</label>
                                <input type="hidden" name="articles[<?= $field_idx ?>][image_url]" id="img_url_<?= $field_idx ?>" value="<?= htmlspecialchars($art['image_url']) ?>">
                                <label class="block w-full py-1.5 text-center text-[9px] font-bold bg-slate-200 hover:bg-slate-350 text-slate-705 rounded-lg cursor-pointer transition-all">
                                    Upload Image
                                ...
                                <div class="text-[8px] text-amber-600 font-bold hidden" id="status_<?= $field_idx ?>">Uploading...</div>
                            </div>

                            <div class="h-16 border border-slate-200 rounded-lg overflow-hidden flex items-center justify-center bg-white" id="preview_<?= $field_idx ?>">
                                <?php if (!empty($art['image_url'])): ?>
                                    <img src="<?= htmlspecialchars($art['image_url']) ?>" alt="Preview" class="h-full object-contain">
                                <?php else: ?>
                                    <span class="text-[9px] text-slate-400 font-medium">No image loaded</span>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
            ...
            <!-- Section 4: Alternating Grid Topics (1-4) -->
            <div class="space-y-6 pt-4">
                <h3 class="text-sm font-bold text-[#20254D] uppercase tracking-wider flex items-center gap-2 pb-2 border-b border-slate-100">
                    <span class="w-1.5 h-4 bg-orange-500 rounded-sm"></span> 4. Alternating Grid Topics (1-4)
                </h3>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <?php foreach ($alt_arts as $idx => $art): ?>
                        <?php $field_idx = $idx + 4; ?>
                        <div class="bg-slate-50/50 border border-slate-200 rounded-xl p-5 space-y-4">
                            <h4 class="text-xs font-bold text-slate-500 uppercase tracking-wider">Alternating Item #<?= $idx + 1 ?></h4>
                            <input type="hidden" name="articles[<?= $field_idx ?>][id]" value="<?= $art['id'] ?>">
                            
                            <div>
                                <label class="block text-xs font-bold text-slate-650 mb-1">Title</label>
                                <input type="text" name="articles[<?= $field_idx ?>][title]" class="w-full px-3 py-2 text-xs bg-white border border-slate-200 rounded-lg text-slate-800 focus:outline-none" required value="<?= htmlspecialchars($art['title']) ?>">
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-slate-650 mb-1">Excerpt / Description</label>
                                <textarea name="articles[<?= $field_idx ?>][excerpt]" rows="2" class="w-full px-3 py-2 text-xs bg-white border border-slate-200 rounded-lg text-slate-800 focus:outline-none" required><?= htmlspecialchars($art['excerpt']) ?></textarea>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="hidden" name="articles[<?= $field_idx ?>][image_url]" id="img_url_<?= $field_idx ?>" value="<?= htmlspecialchars($art['image_url']) ?>">
                                    <label class="block w-full py-2 px-3 text-center text-[10px] font-semibold bg-slate-200 hover:bg-slate-350 text-slate-750 rounded-lg cursor-pointer transition-all">
                                        Upload Image
                                        <input type="file" onchange="uploadImage(this, 'img_url_<?= $field_idx ?>', 'preview_<?= $field_idx ?>', 'status_<?= $field_idx ?>')" class="hidden" accept="image/*">
                                    </label>
