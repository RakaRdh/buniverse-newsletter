<?php $this->load->view('admin/header', ['title' => 'Manage Subscribers']); ?>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        ...
        <form action="<?= base_url('subscribers') ?>" method="GET" class="flex gap-2 mb-6">
            ...
            <?php if (!empty($search)): ?>
                <a href="<?= base_url('subscribers') ?>" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-lg text-sm transition-all">Clear</a>
            <?php endif; ?>
        </form>
            ...
            <table class="w-full border-collapse text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php if (empty($subscribers)): ?>
                        <tr>
                            <td colspan="4" class="p-8 text-center text-slate-400">No subscribers found.</td>
                        </tr>
                    <?php else: ?>
